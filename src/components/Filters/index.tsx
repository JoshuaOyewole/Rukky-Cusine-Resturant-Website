...
const Filters = ({filter, setFilter}: {filter:string, setFilter: any}) => {
  ...
  return (
    ...
    >
      <Button category={{_id: 666, name: "Menu", urlParam: "all", icon: <BiRestaurant />}} filter = {filter} setFilter = {setFilter} />
        {
            Categories.map((category: FoodCategory) =>{
                return <Button key = {category._id} category = {category} filter = {filter} setFilter = {setFilter} />
            })
        }
